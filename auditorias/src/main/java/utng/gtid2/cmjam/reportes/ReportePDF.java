package utng.gtid2.cmjam.reportes;
...
import utng.gtid2.cmjam.modelo.Evaluacion;
import utng.gtid2.cmjam.modelo.Hallazgo;
import utng.gtid2.cmjam.modelo.PlanAccion;
...
public class ReportePDF {
    ...
    public void generar(String rutaSalida,
        ...
        try (PdfWriter writer = new PdfWriter(rutaSalida);
            ...
            doc.add(new Paragraph("Reporte de Auditoría de Igualdad Laboral y No Discriminación")
            ...
            doc.add(new Paragraph("Porcentaje global de cumplimiento (ponderado): " + porcentajeGlobal + " %")
                    .setBold());
            doc.add(new Paragraph(" "));
            ...
            if (rutaImagenGrafica != null && !rutaImagenGrafica.isBlank()) {
                doc.add(new Paragraph("Distribución gráfica de resultados").setBold().setFontSize(13));
            ...
            Table tablaEval = new Table(UnitValue.createPercentArray(new float[]{3, 5, 2, 4}))
                    .useAllAvailableWidth();
            ...
            for (Evaluacion e : evaluaciones) {
                ...
                if ("Cumple".equals(e.getCumplimiento())) celdaCump.setBackgroundColor(new com.itextpdf.kernel.colors.DeviceRgb(198, 239, 206));
                else if ("Parcial".equals(e.getCumplimiento())) celdaCump.setBackgroundColor(new com.itextpdf.kernel.colors.DeviceRgb(255, 235, 156));
                else celdaCump.setBackgroundColor(new com.itextpdf.kernel.colors.DeviceRgb(255, 199, 206));
            ...
            Table tablaHallazgos = new Table(UnitValue.createPercentArray(new float[]{3, 2, 5}))
                    .useAllAvailableWidth();
            ...
            for (PlanAccion p : planes) {
                ...
                tablaPlanes.addCell(p.getFechaCompromiso() != null ? p.getFechaCompromiso().toString() : "Sin fecha");
