        ...
        try (XWPFDocument documento = new XWPFDocument();
            ...
            if (rutaImagenGrafica != null && !rutaImagenGrafica.isBlank()) {
                ...
                try (FileInputStream fis = new FileInputStream(rutaImagenGrafica)) {
                    runImg.addPicture(fis, XWPFDocument.PICTURE_TYPE_PNG, rutaImagenGrafica,
                            Units.toEMU(400), Units.toEMU(260));
                }
                documento.createParagraph();
            }
            ...
            for (int i = 0; i < hallazgos.size(); i++) {
                ...
                PlanAccion planAsociado = planes.stream()
                        .filter(p -> p.getIdHallazgo() == h.getIdHallazgo())
                        .findFirst()
                        .orElse(null);
 
                if (planAsociado != null) {
                    filaTabla.getCell(3).setText(planAsociado.getResponsable());
                    String fechaEstatus = (planAsociado.getFechaCompromiso() != null
                            ? planAsociado.getFechaCompromiso().toString() : "Sin fecha")
                            + " / " + planAsociado.getEstatus();
