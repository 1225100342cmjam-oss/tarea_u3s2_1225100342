...
import utng.gtid2.cmjam.dao.CategoriaDAO;
import utng.gtid2.cmjam.dao.CriterioDAO;
import utng.gtid2.cmjam.dao.EvaluacionDAO;
import utng.gtid2.cmjam.modelo.*;
...
public class CapturaEvaluacionController {
    ...
    public void initialize() {
        ...
        colCumplimiento.setCellValueFactory(new PropertyValueFactory<>("cumplimientoSeleccionado"));
        ...
        colObservaciones.setCellValueFactory(new PropertyValueFactory<>("observacionesCapturadas"));
    ...
    private void cargarCombos() {
        try {
            List<Auditoria> activas = auditoriaDAO.listarTodas().stream()
                    .filter(a -> "En proceso".equals(a.getEstatus()))
    ...
    public void cargarCriterios() {
        ...
        try {
            ...
            List<Criterio> criterios = criterioDAO.obtenerPorCategoria(cmbCategoria.getValue().getIdCategoria());
 
            // Precargar valores ya capturados previamente, si existen, para permitir edicion
            Map<Integer, Evaluacion> yaCapturadas = evaluacionDAO.obtenerPorAuditoria(idAuditoria).stream()
                    .collect(Collectors.toMap(Evaluacion::getIdCriterio, ev -> ev));
 
            for (Criterio c : criterios) {
                Evaluacion existente = yaCapturadas.get(c.getIdCriterio());
                if (existente != null) {
                    c.setCumplimientoSeleccionado(existente.getCumplimiento());
                    c.setObservacionesCapturadas(existente.getObservaciones() != null ? existente.getObservaciones() : "");
                }
            }
