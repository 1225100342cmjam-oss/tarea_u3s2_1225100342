...
public class HallazgosController {
    ...
    public void cargarDatosAuditoria() {
        ...
        try {
            ObservableList<Evaluacion> evaluaciones =
                    FXCollections.observableArrayList(evaluacionDAO.obtenerPorAuditoria(idAuditoria));
            tblEvaluaciones.setItems(evaluaciones);
 
            ObservableList<Hallazgo> hallazgos =
                    FXCollections.observableArrayList(hallazgoDAO.obtenerPorAuditoria(idAuditoria));
    ...
    public void registrarHallazgo() {
        ...
        try {
            Hallazgo h = new Hallazgo(evaluacionSeleccionada.getIdEvaluacion(),
                    chkTipo.getValue(), chkRiesgo.getValue(), txtDescripcionHallazgo.getText());
    ...
    public void guardarPlanAccion() {
        ...
        try {
            PlanAccion p = new PlanAccion(hallazgoSeleccionado.getIdHallazgo(),
                    txtResponsable.getText(), dpFechaCompromiso.getValue(), chkEstatusPlan.getValue());
